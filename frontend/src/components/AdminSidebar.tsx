...
import { Film, Users, FileText, Mail, ShieldCheck, X } from 'lucide-react';
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b admin-sidebar-border">
        <p className="text-xs font-semibold uppercase tracking-widest admin-muted-text">Navigation</p>
      </div>
      <nav className="flex-1 p-3 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`shrink-0 ${isActive ? 'admin-accent-text' : 'admin-muted-text group-hover:admin-accent-text'}`}>
                {item.icon}
              </span>
              <div className="min-w-0">
                <p className={`text-sm font-medium truncate ${isActive ? 'admin-heading' : 'admin-muted-text'}`}>
                  {item.label}
                </p>
                <p className="text-xs admin-muted-text truncate hidden lg:block">{item.description}</p>
              </div>
              {isActive && (
      ...
      <div className="p-4 border-t admin-sidebar-border">
        <p className="text-xs admin-muted-text text-center">Whispers of the White Moon</p>
      </div>
    </div>
  );
        ...
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-64 admin-sidebar flex flex-col shadow-2xl">
            <div className="flex items-center justify-between p-4 border-b admin-sidebar-border">
              <p className="text-xs font-semibold uppercase tracking-widest admin-muted-text">Navigation</p>
              <button onClick={onMobileClose} className="admin-muted-text hover:admin-accent-text transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <nav className="flex-1 p-3 space-y-1">
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={`shrink-0 ${isActive ? 'admin-accent-text' : 'admin-muted-text'}`}>
                      {item.icon}
                    </span>
                    <p className={`text-sm font-medium truncate ${isActive ? 'admin-heading' : 'admin-muted-text'}`}>
                      {item.label}

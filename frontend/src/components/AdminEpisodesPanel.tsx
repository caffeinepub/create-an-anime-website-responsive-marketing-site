...
import { useGetAllEpisodes, useDeleteEpisode } from '../hooks/useQueries';
import { Episode } from '../backend';
import { EpisodeForm } from './EpisodeForm';
import {
  Film, Plus, Pencil, Trash2, Loader2, AlertCircle, RefreshCw, Tv
} from 'lucide-react';
    ...
    <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm admin-muted-text">
              {episodes ? `${episodes.length} episode${episodes.length !== 1 ? 's' : ''}` : 'Loading…'}
            </p>
          </div>
        </div>
      ...
      {error && (
        <div className="flex items-start gap-3 bg-red-500/10 border border-red-500/20 rounded-xl p-4 mb-4">
          ...
          <div>
            <p className="text-sm font-medium text-red-400">Failed to load episodes</p>
      ...
      {!isLoading && !error && sortedEpisodes.length === 0 && (
        <div className="text-center py-16 admin-card rounded-xl">
          ...
          <p className="admin-muted-text text-sm mb-6">Add your first episode to get started.</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="admin-table-header">
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider admin-muted-text">S/E</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider admin-muted-text">Title</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider admin-muted-text hidden md:table-cell">Description</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider admin-muted-text hidden lg:table-cell">Video URL</th>
              ...
              <tbody className="divide-y admin-table-divider">
                {sortedEpisodes.map((episode) => (
                  <tr key={episode.id} className="admin-table-row transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <p className="text-sm font-medium admin-heading truncate max-w-[180px]">{episode.title}</p>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <p className="text-xs admin-muted-text truncate max-w-[200px]">{episode.description}</p>
                    </td>
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <p className="text-xs admin-muted-text font-mono truncate max-w-[160px]">
                        {episode.videoUrl || <span className="italic opacity-50">No URL</span>}
                      </p>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent className="admin-dialog">
                            <AlertDialogHeader>
                              <AlertDialogTitle className="admin-heading">Delete Episode</AlertDialogTitle>
